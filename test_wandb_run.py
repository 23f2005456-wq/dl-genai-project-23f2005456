...
# Import everything from notebook cells
import gc
import re
import warnings
import random
...
def apk(a: int, p: list, k: int = 3) -> float:
    if len(p) > k: p = p[:k]
    s, h = 0.0, 0
...
study.optimize(objective_nd, n_trials=100)
...
print("📦 Uploaded submission.csv to W&B as Artifact!")
wandb.finish()
